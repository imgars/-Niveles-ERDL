...
app.get('/api/rankcard/verify', async (req, res) => {
  try {
    ...
    if (!verified) {
      return res.status(401).json({ error: 'Token inválido o expirado. Usa /rankcard link en Discord para obtener uno nuevo.' });
    }
    ...
    console.error('Error en /api/rankcard/verify:', error);
    res.status(500).json({ error: 'Error al verificar token' });
  }
});

app.post('/api/rankcard/calculate-cost', express.json(), async (req, res) => {
  try {
    const { token, config } = req.body;
    const { verifyToken, validateRankcardConfig, checkVIPBoosterRoles, calculateRankcardCost } = await import('./utils/rankcardService.js');
    ...
    const roles = checkVIPBoosterRoles(member);
    const validation = validateRankcardConfig(config || {}, roles);
    if (!validation.valid) return res.status(400).json({ error: validation.error });

    const total = calculateRankcardCost(validation.sanitized, roles);
    res.json({ total, breakdown: { base: 7500, extras: total - 7500 } });
  } catch (error) {
    console.error('Error en /api/rankcard/calculate-cost:', error);
    res.status(500).json({ error: 'Error al calcular costo' });
  }
});

app.post('/api/rankcard/preview', express.json(), async (req, res) => {
  try {
    const { token, config } = req.body;
    const { verifyToken, validateRankcardConfig, checkVIPBoosterRoles } = await import('./utils/rankcardService.js');
    ...
    res.status(500).json({ error: 'Error al generar previsualización' });
  }
});

app.post('/api/rankcard/purchase', async (req, res) => {
  try {
    const { token, config } = req.body;
    const { verifyToken, validateRankcardConfig, checkVIPBoosterRoles, calculateRankcardCost } = await import('./utils/rankcardService.js');
    const { getEconomy, removeLagcoins } = await import('./utils/mongoSync.js');

    const verified = verifyToken(token);
    if (!verified) {
      return res.status(401).json({ error: 'Token inválido o expirado. Usa /rankcard link en Discord.' });
    }
    ...
    const totalCost = calculateRankcardCost(validation.sanitized, roles);

    let economy = await getEconomy(guildId, userId);
    const balance = economy?.lagcoins || 0;
    if (balance < totalCost) {
      return res.status(402).json({
        error: 'Lagcoins insuficientes',
        required: totalCost,
        balance
      });
    }

    const result = await removeLagcoins(guildId, userId, totalCost, 'rankcard_custom');
    if (!result) {
      return res.status(402).json({ error: 'No se pudo procesar el pago. Intenta de nuevo.' });
    }
    ...
    res.json({
      ...
      message: `Rankcard personalizada guardada. Costo: ${totalCost} Lagcoins`
    });
  } catch (error) {
